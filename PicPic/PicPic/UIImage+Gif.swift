...
//            print("SwiftGif: Cannot turn image named \"\(name)\" into NSData")
    ...
    public class func animatedInfoWithSource(source: CGImageSource, inout frames:[UIImage], inout duration:NSTimeInterval) {
        let count = CGImageSourceGetCount(source)
        var images = [CGImageRef]()
        var delays = [Int]()
        ...
        print("_duration : ",Double(_duration))
        NSLog("double duration : %.3lf",Double(_duration)/1000)
        duration = NSTimeInterval(Double(_duration)/1000)
