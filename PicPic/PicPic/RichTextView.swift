    ...
    public func initCommentStyle(textView:UITextView,url:String?) {
        ...
        for(var i=0;i<str_arr.count;i++) {
            ...
            if let urlString = url {
                let url = NSURL(string: imageURL.imageurl(urlString))
                if let imagedata = NSData(contentsOfURL: url!){
                    ...
                    var scaleFactor = oldWidth/(textView.frame.size.width - 10)
                    ...
                    para.replaceCharactersInRange(NSMakeRange(0, 1), withAttributedString: attrStringWithImage)
                }
            }
