...
class GoogleprofileViewController: UIViewController ,UIImagePickerControllerDelegate,UINavigationControllerDelegate{
    ...
    @IBAction func join(sender: AnyObject) {
        filename = enc(userData["email"].stringValue)+userData["regist_day"].stringValue+".jpg"
        ...
        self.appdelegate.userData = self.userData
        ...
        self.appdelegate.doIt(201, message: self.appdelegate.userData) { (readData) -> () in
            if readData["msg"].string! == "success" {
                ...
                self.appdelegate.standardUserDefaults.setValue(self.enc(self.userData["email"].stringValue), forKey: "id")
                self.appdelegate.standardUserDefaults.setValue(self.enc(self.userData["password"].stringValue), forKey: "password")
                self.appdelegate.standardUserDefaults.setValue(self.enc(self.userData["register_form"].stringValue), forKey: "register_form")
...
//            image2 = image2.imageRotatedByDegrees1(180.0)
            ...
            print("image down")
        } else if image.imageOrientation == UIImageOrientation.LeftMirrored {
            print("iamge mirrored")
            if #available(iOS 9.0, *) {
                image2 = UIImage.imageFlippedForRightToLeftLayoutDirection(image2)()
            } else {
                image2 = UIImage(CIImage: image2.CIImage!, scale: image2.scale, orientation: .LeftMirrored)
            }
        }
